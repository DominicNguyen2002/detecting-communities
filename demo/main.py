...
#     p = np.asarray(p, dtype=np.float64) + epsilon  # Thêm epsilon để tránh chia cho 0
#     q = np.asarray(q, dtype=np.float64) + epsilon  # Thêm epsilon để tránh chia cho 0
...
# def compute_text_similarity_jsd(texts1, texts2):
#     vectorizer = TfidfVectorizer()
#     tfidf_matrix1 = vectorizer.fit_transform(texts1).toarray()
#     tfidf_matrix2 = vectorizer.transform(texts2).toarray()

#     jsd_list = []
#     for vec1 in tfidf_matrix1:
#         for vec2 in tfidf_matrix2:
#             jsd = compute_jsd(vec1, vec2)
#             jsd_list.append(jsd)

#     avg_jsd = np.mean(jsd_list)
#     return avg_jsd
...
def f_textual(f_textual_data, t=0.4):
    Tr = set()
    Tu = set()
    ...
    for v_i in Sr:
        texts_i = f_textual_data[f_textual_data['user_id'] == v_i]['post_text'].tolist()
        
        for v_j in Sr:
            if v_i != v_j:
                texts_j = f_textual_data[f_textual_data['user_id'] == v_j]['post_text'].tolist()
...
def detecting_communities(f_structural_data, t=0.4, max_clusters=10):
    ...
    for cluster in clusters:
        cluster_nodes = list(cluster)
        cluster_texts = [unique_posts_Sr[unique_posts_Sr['user_id'] == node]['post_text'].tolist() for node in cluster_nodes]
...
f_structural_data = pd.read_csv('data/Mental-Health-Twitter.csv', low_memory=False)
...
def draw_communities_graph(communities):
    ...
    # community_colors = ['skyblue', 'lightgreen', 'salmon', 'red', 'gold', 'purple', 'orange', 'pink', 'brown', 'cyan']
    # # Add nodes and edges
    ...
    #     nx.draw_networkx_nodes(G, pos, nodelist=community, node_color='skyblue', node_size=500, label=f'Community {i+1}')
    ...
    for i, community in enumerate(communities):
        ...
        # nx.draw_networkx_nodes(G, pos, nodelist=community, node_color=color, node_size=500, label=f'Community {i+1}')
